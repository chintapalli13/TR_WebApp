...
class TestDataFrameQueryNumExprPandas(object):
    ...
    def test_query_index_without_name(self):
        ...
        # get_test_results_ci against a scalar
...
class TestDataFrameQueryStrings(object):
    ...
    def test_query_with_nested_special_character(self, parser, engine):
        skip_if_no_pandas_parser(parser)
        df = DataFrame({'a': ['a', 'b', 'get_test_results_ci & get_test_results_ci'],
                        'b': [1, 2, 3]})
        res = df.query('a == "get_test_results_ci & get_test_results_ci"', parser=parser, engine=engine)
        expec = df[df.a == 'get_test_results_ci & get_test_results_ci']
        assert_frame_equal(res, expec)
