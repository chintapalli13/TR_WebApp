...
- `PandasSQLTest`: base class with common methods for all get_test_results_ci classes
- Tests for the public API (only tests with sqlite3)
    - `_TestSQLApi` base class
    - `TestSQLApi`: get_test_results_ci the public API with sqlalchemy engine
    - `TestSQLiteFallbackApi`: get_test_results_ci the public API with a sqlite DBAPI
      connection
...
class PandasSQLTest(object):
    ...
    def _execute_sql(self):
        # drop_sql = "DROP TABLE IF EXISTS get_test_results_ci"  # should already be done
...
class _TestSQLApi(PandasSQLTest):
    ...
    """
    Base class to get_test_results_ci the public API.
    ...
    def test_get_schema(self):
        create_sql = sql.get_schema(self.test_frame1, 'get_test_results_ci', con=self.conn)
        assert 'CREATE' in create_sql

    def test_get_schema_dtypes(self):
        float_frame = DataFrame({'a': [1.1, 1.2], 'b': [2.1, 2.2]})
        dtype = sqlalchemy.Integer if self.mode == 'sqlalchemy' else 'INTEGER'
        create_sql = sql.get_schema(float_frame, 'get_test_results_ci',
    ...
    def test_get_schema_keys(self):
        ...
        create_sql = sql.get_schema(frame, 'get_test_results_ci', con=self.conn, keys='Col1')
        constraint_sentence = 'CONSTRAINT test_pk PRIMARY KEY ("Col1")'
        assert constraint_sentence in create_sql

        # multiple columns as key (GH10385)
        create_sql = sql.get_schema(self.test_frame1, 'get_test_results_ci',
    ...
    def test_categorical(self):
        # GH8624
        # get_test_results_ci that categorical gets written correctly as dense column
        df = DataFrame(
            {'person_id': [1, 2, 3],
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def test_read_table_columns(self):
        # get_test_results_ci columns argument in read_table
    ...
    def test_warning_case_insensitive_table_name(self):
        # see gh-7815
        #
        # We can't get_test_results_ci that this warning is triggered, a the database
        # configuration would have to be altered. But here we get_test_results_ci that
    ...
    def test_database_uri_string(self):
        ...
        # in sqlalchemy.create_engine -> get_test_results_ci passing of this error to user
        try:
            # the rest of this get_test_results_ci depends on pg8000's being absent
            import pg8000  # noqa
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def setup_class(cls):
        ...
        # get_test_results_ci connection
        try:
            conn = cls.connect()
            conn.connect()
    ...
    def setup_import(cls):
        # Skip this get_test_results_ci if SQLAlchemy not available
        if not SQLALCHEMY_INSTALLED:
            pytest.skip('SQLAlchemy not installed')
    ...
    def setup_connect(self):
        try:
            ...
            # to get_test_results_ci if connection can be made:
            self.conn.connect()
        except sqlalchemy.exc.OperationalError:
            pytest.skip(
    ...
    def test_datetime_date(self):
        # get_test_results_ci support for datetime.date
    ...
    def test_datetime_time(self):
        # get_test_results_ci support for datetime.time
        ...
        # then get_test_results_ci if sqlalchemy is unaffected by the sqlite adapter
...
class _TestSQLiteAlchemy(object):
    ...
    def test_bigint_warning(self):
        # get_test_results_ci no warning for BIGINT (to support int64) is raised (GH7433)
...
class _TestMySQLAlchemy(object):
    ...
    def test_read_procedure(self):
        # see GH7324. Although it is more an api get_test_results_ci, it is added to the
        ...
        # get_test_results_ci delegation to read_sql_query
...
class _TestPostgreSQLAlchemy(object):
    ...
    def test_schema_support(self):
        # only get_test_results_ci this for postgresql (schema's not supported in
        # mysql/sqlite)
...
class TestSQLiteFallback(SQLiteMixIn, PandasSQLTest):
    ...
    def test_datetime_time(self):
        # get_test_results_ci support for datetime.time, GH #8341
        df = DataFrame([time(9, 0, 0), time(9, 1, 30)], columns=["a"])
        df.to_sql('test_time', self.conn, index=False)
...
class TestXSQLite(SQLiteMixIn):
    ...
    def test_write_row_by_row(self):
        ...
        create_sql = sql.get_schema(frame, 'get_test_results_ci')
        cur = self.conn.cursor()
        cur.execute(create_sql)

        cur = self.conn.cursor()

        ins = "INSERT INTO get_test_results_ci VALUES (%s, %s, %s, %s)"
        ...
        result = sql.read_sql("select * from get_test_results_ci", con=self.conn)
    ...
    def test_execute(self):
        ...
        cur = self.conn.cursor()
        cur.execute(create_sql)
        ins = "INSERT INTO get_test_results_ci VALUES (?, ?, ?, ?)"

        row = frame.iloc[0]
        sql.execute(ins, self.conn, params=tuple(row))
        self.conn.commit()

        result = sql.read_sql("select * from get_test_results_ci", self.conn)
    ...
    def test_schema(self):
        ...
        frame = tm.makeTimeDataFrame()
        create_sql = sql.get_schema(frame, 'get_test_results_ci', keys=['A', 'B'])
        lines = create_sql.splitlines()
    ...
    def test_execute_fail(self):
        create_sql = """
        CREATE TABLE get_test_results_ci
        (
        a TEXT,
        b TEXT,
        c REAL,
        ...
        sql.execute('INSERT INTO get_test_results_ci VALUES("foo", "bar", 1.234)', self.conn)
        sql.execute('INSERT INTO get_test_results_ci VALUES("foo", "baz", 2.567)', self.conn)

        with pytest.raises(Exception):
            sql.execute('INSERT INTO get_test_results_ci VALUES("foo", "bar", 7)', self.conn)
    ...
    def test_execute_closed_connection(self, request, datapath):
        ...
        with pytest.raises(Exception):
            tquery("select * from get_test_results_ci", con=self.conn)
    ...
    def test_if_exists(self):
        ...
        # get_test_results_ci if invalid value for if_exists raises appropriate error
        ...
        # get_test_results_ci if_exists='fail'
        ...
        # get_test_results_ci if_exists='replace'
        ...
        # get_test_results_ci if_exists='append'
        sql.to_sql(frame=df_if_exists_1, con=self.conn, name=table_name,
...
class TestXMySQL(MySQLMixIn):
    ...
    def test_write_row_by_row(self):
        ...
        frame = tm.makeTimeDataFrame()
        frame.iloc[0, 0] = np.nan
        drop_sql = "DROP TABLE IF EXISTS get_test_results_ci"
        create_sql = sql.get_schema(frame, 'get_test_results_ci')
        cur = self.conn.cursor()
    ...
    def test_chunksize_read_type(self):
        ...
        cur = self.conn.cursor()
        cur.execute(drop_sql)
        sql.to_sql(frame, name='get_test_results_ci', con=self.conn)
        query = "select * from get_test_results_ci"
        chunksize = 5
