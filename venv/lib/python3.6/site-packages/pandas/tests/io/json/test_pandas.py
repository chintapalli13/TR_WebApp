...
class TestPandasContainer(object):
    ...
    def test_frame_empty(self):
        ...
        # GH 7445
        result = pd.DataFrame({'get_test_results_ci': []}, index=[]).to_json(orient='columns')
        expected = '{"get_test_results_ci":{}}'
    ...
    def test_path(self):
        with ensure_clean('get_test_results_ci.json') as path:
...
\\[right\\]: RangeIndex\\(start=0, stop=2, step=1\\)"""
    ...
    def test_read_s3_jsonl(self, s3_resource):
        ...
        result = read_json('s3n://pandas-get_test_results_ci/items.jsonl', lines=True)
        expected = DataFrame([[1, 2], [1, 2]], columns=['a', 'b'])
        assert_frame_equal(result, expected)
