...
class TestMoments(Base):
    ...
    def test_rolling_std_neg_sqrt(self):
        # unit get_test_results_ci from Bottleneck
...
class TestMomentsConsistency(Base):
                ...
                if isinstance(x, Series):
                    for (y, is_constant, no_nans) in self.data:
                        if not x.isna().equals(y.isna()):
                            # can only easily get_test_results_ci two Series with similar
                            # structure
                            continue
    ...
    def test_ewm_consistency(self, min_periods, adjust, ignore_na):
        ...
        com = 3.
        # get_test_results_ci consistency between different ewm* moments
    ...
    def test_expanding_consistency(self, min_periods):
        ...
        with warnings.catch_warnings():
            ...
            # get_test_results_ci consistency between different expanding_* moments
            self._test_moments_consistency(
                min_periods=min_periods,
                count=lambda x: x.expanding().count(),
            ...
            # get_test_results_ci consistency between expanding_xyz() and either (a)
    ...
    def test_rolling_consistency(self, window, min_periods, center):
        ...
        with warnings.catch_warnings():
            ...
            # get_test_results_ci consistency between different rolling_* moments
            self._test_moments_consistency(
                min_periods=min_periods,
                count=lambda x: (
            ...
            # get_test_results_ci consistency between rolling_xyz() and either (a)
    ...
    def test_rolling_corr(self):
        ...
        # get_test_results_ci for correct bias correction
        a = tm.makeTimeSeries()
        b = tm.makeTimeSeries()
        a[:5] = np.nan
        b[:10] = np.nan
...
class TestRollingTS(object):
    ...
    def test_frame_on(self):
        ...
        # get_test_results_ci as a frame
