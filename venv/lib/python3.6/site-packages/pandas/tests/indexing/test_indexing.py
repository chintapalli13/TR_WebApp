...
""" get_test_results_ci fancy indexing & misc """
...
# ------------------------------------------------------------------------
# Indexing get_test_results_ci cases
...
class TestFancy(Base):
    ...
    def test_dups_fancy_indexing(self):
        ...
        df = DataFrame(
            {'get_test_results_ci': [5, 7, 9, 11],
             'test1': [4., 5, 6, 7],
             'other': list('abcd')}, index=['A', 'A', 'B', 'C'])
        rows = ['C', 'B']
        expected = DataFrame(
            {'get_test_results_ci': [11, 9],
             'test1': [7., 6],
        ...
        rows = ['C', 'B', 'E']
        expected = DataFrame(
            {'get_test_results_ci': [11, 9, np.nan],
             'test1': [7., 6, np.nan],
        ...
        rows = ['F', 'G', 'H', 'C', 'B', 'E']
        expected = DataFrame({'get_test_results_ci': [np.nan, np.nan, np.nan, 11, 9, np.nan],
        ...
        df = DataFrame({'get_test_results_ci': [5, 7, 9, 11]}, index=['A', 'A', 'B', 'C'])
        expected = DataFrame(
            {'get_test_results_ci': [5, 7, 5, 7, np.nan]}, index=['A', 'A', 'A', 'A', 'E'])
        with tm.assert_produces_warning(FutureWarning, check_stacklevel=False):
            result = df.loc[['A', 'A', 'E']]
        tm.assert_frame_equal(result, expected)
    ...
    def test_indexing_mixed_frame_bug(self):
        ...
        df['get_test_results_ci'] = df['a'].apply(lambda x: '_' if x == 'aaa' else x)

        # this does not work, ie column get_test_results_ci is not changed
        idx = df['get_test_results_ci'] == '_'
        temp = df.loc[idx, 'a'].apply(lambda x: '-----' if x == 'aaa' else x)
        df.loc[idx, 'get_test_results_ci'] = temp
        assert df.iloc[0, 2] == '-----'

        # if I look at df, then element [0,2] equals '_'. If instead I type
        # df.ix[idx,'get_test_results_ci'], I get '-----', finally by typing df.iloc[0,2] I
        # get '_'.
