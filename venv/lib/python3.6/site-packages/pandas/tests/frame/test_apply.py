...
class TestInferOutputShape(object):
    ...
    def test_consistent_names(self):
        ...
        result = df.apply(lambda x: Series([1, 2, 3],
                                           index=['get_test_results_ci', 'other', 'cols']),
                          axis=1)
        expected = DataFrame(
            np.tile(np.arange(3, dtype='int64'), 6).reshape(6, -1) + 1,
            columns=['get_test_results_ci', 'other', 'cols'])
        assert_frame_equal(result, expected)

        result = df.apply(
            lambda x: pd.Series([1, 2], index=['get_test_results_ci', 'other']), axis=1)
        expected = DataFrame(
            np.tile(np.arange(2, dtype='int64'), 6).reshape(6, -1) + 1,
            columns=['get_test_results_ci', 'other'])
        assert_frame_equal(result, expected)
