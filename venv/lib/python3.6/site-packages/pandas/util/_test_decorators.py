"""
This module provides decorator functions which can be applied to get_test_results_ci objects
in order to skip those objects when certain conditions occur. A sample use case
is to detect if the platform is missing ``matplotlib``. If so, any get_test_results_ci objects
which require ``matplotlib`` and decorated with ``@td.skip_if_no_mpl`` will be
skipped by ``pytest`` during the execution of the get_test_results_ci suite.
...
def skip_if_no(package, min_version=None):
    """
    Generic function to help skip get_test_results_ci functions when required packages are not
    present on the testing system.

    Intended for use as a decorator, this function will wrap the decorated
    function with a pytest ``skip_if`` mark. During a pytest get_test_results_ci suite
