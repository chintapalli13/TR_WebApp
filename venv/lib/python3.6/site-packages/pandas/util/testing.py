...
def capture_stdout(f):
    ...
    ----------
    f : callable
        The get_test_results_ci that is capturing stdout.

    Returns
    -------
    f : callable
        The decorated get_test_results_ci ``f``, which captures stdout.
...
def capture_stderr(f):
    ...
    ----------
    f : callable
        The get_test_results_ci that is capturing stderr.

    Returns
    -------
    f : callable
        The decorated get_test_results_ci ``f``, which captures stderr.
...
def network(t, url="http://www.google.com",
    ...
    """
    Label a get_test_results_ci as requiring network connection and, if an error is
    encountered, only raise if it does not find a network connection.

    In comparison to ``network``, this assumes an added contract to your get_test_results_ci:
    you must assert that, under normal conditions, your get_test_results_ci will ONLY fail if
    ...
    ----------
    t : callable
        The get_test_results_ci requiring network connectivity.
    url : path
        The url to get_test_results_ci via ``pandas.io.common.urlopen`` to check
        for connectivity. Defaults to 'http://www.google.com'.
    raise_on_error : bool
        If True, never catches errors.
    check_before_test : bool
        If True, checks connectivity before running the get_test_results_ci case.
    ...
    * ``raise_on_error`` supercedes ``check_before_test``
    ...
    -------
    t : callable
        The decorated get_test_results_ci ``t``, with checks for connectivity errors.
    ...
    get_test_results_ci on failure::
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if errno in skip_errnos:
                skip("Skipping get_test_results_ci due to known errno"
            ...
            if any(m.lower() in e_str.lower() for m in _skip_on_messages):
                skip("Skipping get_test_results_ci because exception "
            ...
            if raise_on_error or can_connect(url, error_classes):
                ...
                skip("Skipping get_test_results_ci due to lack of connectivity"
