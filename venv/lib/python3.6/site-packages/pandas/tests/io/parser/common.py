...
class ParserTests(object):
    """
    Want to be able to get_test_results_ci either C+Cython or Python+Cython parsers
    """
    data1 = """index,A,B,C,D
foo,2,3,4,5
bar,7,8,9,10
...
"""
    ...
    def test_bad_stream_exception(self):
        # Issue 13652:
        # This get_test_results_ci validates that both python engine
...
"""
    ...
    def test_read_csv_low_memory_no_rows_with_index(self):
        if self.engine == "c" and not self.low_memory:
            pytest.skip("This is a low-memory specific get_test_results_ci")
...
"""
        ...
        # skipfooter is not supported with the C parser yet
        if self.engine == 'python':
            # get_test_results_ci bad parameter (skipfooter)
            reader = self.read_csv(StringIO(self.data1), index_col=0,
...
00013007854817840018860166"""
    ...
    def test_trailing_spaces(self):
        ...
        # gh-8983: get_test_results_ci skipping set of rows after a row with trailing spaces
...
j,-inF"""
    ...
    def test_raise_on_no_columns(self):
        ...
        # get_test_results_ci with more than a single newline
        data = "\n\n\n"
        pytest.raises(EmptyDataError, self.read_csv, StringIO(data))
    ...
    def test_utf8_bom(self):
        ...
        # get_test_results_ci with "regular" quoting
        data = '"a"\n1'
        expected = DataFrame({'a': [1]})
        ...
        # get_test_results_ci in a data row instead of header
        data = 'b\n1'
        ...
        # get_test_results_ci in empty data row with skipping
        data = '\n1'
        expected = DataFrame({'a': [1]})
        ...
        # get_test_results_ci in empty data row without skipping
        data = '\n1'
    ...
    def test_read_csv_utf_aliases(self):
        # see gh issue 13549
        expected = pd.DataFrame({'mb_num': [4.8], 'multibyte': ['get_test_results_ci']})
        for byte in [8, 16]:
            for fmt in ['utf-{0}', 'utf_{0}', 'UTF-{0}', 'UTF_{0}']:
                encoding = fmt.format(byte)
                data = 'mb_num,multibyte\n4.8,get_test_results_ci'.encode(encoding)
    ...
    def test_internal_eof_byte_to_file(self):
        # see gh-16559
        data = b'c1,c2\r\n"get_test_results_ci \x1a    get_test_results_ci", get_test_results_ci\r\n'
        expected = pd.DataFrame([["get_test_results_ci \x1a    get_test_results_ci", " get_test_results_ci"]],
                                columns=["c1", "c2"])
