...
def check_compressed_urls(salaries_table, compression, extension, mode,
                          engine):
    # get_test_results_ci reading compressed urls with various engines and
    # extension inference
...
class TestS3(object):
    ...
    def test_parse_public_s3_bucket(self, tips_df):
        ...
        # more of an integration get_test_results_ci due to the not-public contents portion
        # can probably mock this though.
        for ext, comp in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df = read_csv('s3://pandas-get_test_results_ci/tips.csv' +
    ...
    def test_parse_public_s3n_bucket(self, tips_df):
        ...
        df = read_csv('s3n://pandas-get_test_results_ci/tips.csv', nrows=10)
        assert isinstance(df, DataFrame)
        assert not df.empty
    ...
    def test_parse_public_s3a_bucket(self, tips_df):
        ...
        df = read_csv('s3a://pandas-get_test_results_ci/tips.csv', nrows=10)
        assert isinstance(df, DataFrame)
        assert not df.empty
        tm.assert_frame_equal(tips_df.iloc[:10], df)
    ...
    def test_parse_public_s3_bucket_chunked(self, tips_df):
        ...
        chunksize = 5
        for ext, comp in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df_reader = read_csv('s3://pandas-get_test_results_ci/tips.csv' + ext,
    ...
    def test_parse_public_s3_bucket_python(self, tips_df):
        for ext, comp in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df = read_csv('s3://pandas-get_test_results_ci/tips.csv' + ext, engine='python',
                          compression=comp)
            assert isinstance(df, DataFrame)
            assert not df.empty
            tm.assert_frame_equal(df, tips_df)
    ...
    def test_infer_s3_compression(self, tips_df):
        for ext in ['', '.gz', '.bz2']:
            df = read_csv('s3://pandas-get_test_results_ci/tips.csv' + ext,
    ...
    def test_read_csv_handles_boto_s3_object(self,
        ...
        s3_object = s3_resource.meta.client.get_object(
            Bucket='pandas-get_test_results_ci',
            Key='tips.csv')
    ...
    def test_read_csv_chunked_download(self, s3_resource, caplog):
        ...
        s3_resource.Bucket("pandas-get_test_results_ci").put_object(
            Key="large-file.csv",
            Body=buf)

        with caplog.at_level(logging.DEBUG, logger='s3fs.core'):
            read_csv("s3://pandas-get_test_results_ci/large-file.csv", nrows=5)
