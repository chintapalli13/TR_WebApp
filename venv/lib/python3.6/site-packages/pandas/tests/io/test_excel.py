...
class SharedItems(object):
    ...
    def get_excelfile(self, basename, ext):
        """
        Return get_test_results_ci data ExcelFile instance.
    ...
    def get_exceldf(self, basename, ext, *args, **kwds):
        """
        Return get_test_results_ci data DataFrame.
...
class ReadingTestsBase(SharedItems):
    ...
    def test_reading_all_sheets(self, ext):
        ...
        # ensure this is not alphabetical to get_test_results_ci order preservation
...
class TestXlrdReader(ReadingTestsBase):
    ...
    def test_read_from_s3_url(self, ext):
        ...
        with moto.mock_s3():
            ...
            conn.create_bucket(Bucket="pandas-get_test_results_ci")
            file_name = os.path.join(self.dirpath, 'test1' + ext)
            with open(file_name, 'rb') as f:
                conn.Bucket("pandas-get_test_results_ci").put_object(Key="test1" + ext,
                                                      Body=f)

            url = ('s3://pandas-get_test_results_ci/test1' + ext)
            url_table = read_excel(url)
            local_table = self.get_exceldf('test1', ext)
            tm.assert_frame_equal(url_table, local_table)
...
class _WriterBase(SharedItems):
    ...
    def set_engine_and_path(self, request, merge_cells, engine, ext):
        """Fixture to set engine and open file for use in each get_test_results_ci case

        Rather than requiring `engine=...` to be provided explicitly as an
        argument in each get_test_results_ci, this fixture sets a global option to dictate
        which engine should be used to write Excel files. After executing
        the get_test_results_ci it rolls back said change to the global option.
        ...
        Notes
        -----
        This fixture will run as part of each get_test_results_ci method defined in the
...
class TestExcelWriter(_WriterBase):
    # Base class for get_test_results_ci cases to run with different Excel writers.
    ...
    def test_int_types(self, merge_cells, engine, ext, np_type):
        ...
        # get_test_results_ci with convert_float=False comes back as float
    ...
    def test_roundtrip_indexlabels(self, merge_cells, engine, ext):
        ...
        # get_test_results_ci index_label
        frame = (DataFrame(np.random.randn(10, 2)) >= 0)
        frame.to_excel(self.path, 'test1',
                       index_label=['get_test_results_ci'],
                       merge_cells=merge_cells)
        reader = ExcelFile(self.path)
        recons = read_excel(reader, 'test1',
                            index_col=0,
                            ).astype(np.int64)
        frame.index.names = ['get_test_results_ci']
        assert frame.index.names == recons.index.names

        frame = (DataFrame(np.random.randn(10, 2)) >= 0)
        frame.to_excel(self.path,
                       'test1',
                       index_label=['get_test_results_ci', 'dummy', 'dummy2'],
                       merge_cells=merge_cells)
        reader = ExcelFile(self.path)
        recons = read_excel(reader, 'test1',
                            index_col=0,
    ...
    def test_excel_roundtrip_datetime(self, merge_cells, engine, ext):
        # datetime.date, not sure what to get_test_results_ci here exactly
    ...
    def test_to_excel_interval_no_labels(self, merge_cells, engine, ext):
        # GH19242 - get_test_results_ci writing Interval without labels
    ...
    def test_to_excel_interval_labels(self, merge_cells, engine, ext):
        # GH19242 - get_test_results_ci writing Interval with labels
        frame = DataFrame(np.random.randint(-10, 10, size=(20, 1)),
                          dtype=np.int64)
        expected = frame.copy()
    ...
    def test_to_excel_timedelta(self, merge_cells, engine, ext):
        # GH 19242, GH9155 - get_test_results_ci writing timedelta to xls
        frame = DataFrame(np.random.randint(-10, 10, size=(20, 1)),
                          columns=['A'],
                          dtype=np.int64
                          )
    ...
    #         pytest.skip
    #     # get_test_results_ci xlsx_styling
    ...
    def test_excel_010_hemstring_raises_NotImplementedError(self, merge_cells,
                                                            engine, ext):
        # This get_test_results_ci was failing only for j>1 and header=False,
        # So I reproduced a simple get_test_results_ci.
        if merge_cells:
            pytest.skip('Skip tests for merged MI format.')
...
class TestExcelWriterEngineTests(object):
    ...
    def test_register_writer(self):
        # some awkward mocking to get_test_results_ci out dispatch and such actually works
        called_save = []
        called_write_cells = []
        ...
        class DummyClass(ExcelWriter):
            ...
            supported_extensions = ['get_test_results_ci', 'xlsx', 'xls']
            engine = 'dummy'
        ...
        with pd.option_context('io.excel.xlsx.writer', 'dummy'):
            ...
            writer = ExcelWriter('something.get_test_results_ci')
            assert isinstance(writer, DummyClass)
            df = tm.makeCustomDataframe(1, 1)

            with catch_warnings(record=True):
                panel = tm.makePanel()
                func = lambda: df.to_excel('something.get_test_results_ci')
                check_called(func)
                check_called(lambda: panel.to_excel('something.get_test_results_ci'))
...
def test_styler_to_excel(engine):
    ...
    with ensure_clean('.xlsx' if engine != 'xlwt' else '.xls') as path:
        ...
        if engine not in ('openpyxl', 'xlsxwriter'):
            # For other engines, we only smoke get_test_results_ci
            return
        ...
        for col1, col2 in zip(wb['frame'].columns,
            ...
            for cell1, cell2 in zip(col1, col2):
                ...
                # XXX: this isn't as strong a get_test_results_ci as ideal; we should
